...
3. Создатель лобби (или любой, кто ещё раз наберёт !spy_start) пишет !spy_start,
   чтобы начать раунд.
...
5. Любой участник может завершить игру командой !spy_end — бот объявит,
   кто был шпионом и какое было слово/локация.
...
!spy_start [минуты]  — начать игру (по умолчанию таймер 8 минут)
!spy_end             — завершить игру и раскрыть роли
...
async def spy_help(ctx: commands.Context):
    await ctx.send(
        ...
        "`!spy_start [минуты]` — начать игру (по умолчанию 8 минут)\n"
        "`!spy_end` — завершить игру и раскрыть роли\n\n"
...
async def spy_start(ctx: commands.Context, minutes: int = DEFAULT_ROUND_MINUTES):
    game = get_game(ctx.channel.id)
    if not game:
        await ctx.send("Сначала создайте лобби командой `!spy_create`.")
        return
    ...
    if minutes <= 0 or minutes > 60:
        await ctx.send("Укажи разумную длительность раунда: от 1 до 60 минут.")
        return
...
async def spy_end(ctx: commands.Context):
    ...
    await ctx.send(
        ...
        "Новую игру можно начать командой `!spy_create`."
    )
    games.pop(ctx.channel.id, None)
